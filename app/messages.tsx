...
import { MessageLog, getMessageLogs, deleteMessageLogs, deleteAllMessageLogs } from '../src/utils/storage';
...
export default function MessagesScreen() {
  ...
  const fetchLogs = async () => {
    try {
      const fetchedLogs = await getMessageLogs();
  ...
  const filterLogsByDate = (logs: MessageLog[], selectedDate: Date) => {
  ...
  const handleDeleteAll = () => {
    Alert.alert('Confirm Delete All', 'Are you sure you want to delete all message logs?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            ...
            setLogs([]);
            setFilteredLogs([]);
            Alert.alert('Success', 'All message logs deleted successfully!');
          } catch (err) {
  ...
  const handleBulkDelete = () => {
    ...
    Alert.alert('Confirm Bulk Delete', `Are you sure you want to delete ${selectedLogs.length} log(s)?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await deleteMessageLogs(selectedLogs);
            setSelectedLogs([]);
  ...
  const renderLog = ({ item }: { item: MessageLog }) => (
    <View style={styles.logItem}>
      ...
      <View style={styles.logInfo}>
        ...
        <Text style={styles.logText}>Phone: {item.phoneNumber}</Text>
        <Text style={styles.logText}>Message: {item.message}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      {filteredLogs.length === 0 ? (
        <Text style={styles.noDataText}>No message logs for this date</Text>
      ) : (
        <FlatList
          data={filteredLogs}
